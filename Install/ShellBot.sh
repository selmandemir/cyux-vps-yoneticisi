...
#	SCRIPT:				ShellBot.sh
#	VERSİYON:			6.4.0
#	GELİŞTİREN:	        Juliano Santos [SHAMAN]
#	SAYFA:				http://www.shellscriptx.blogspot.com.br
#	FACEBOOK:			https://www.facebook.com/shellscriptx
#	GITHUB:				https://github.com/shellscriptx
# 	İLETİŞİM:			shellscriptx@gmail.com
#
#	AÇIKLAMA:			ShellBot, TELEGRAM platformunda bot oluşturmayı kolaylaştırmak için
# 					    tasarlanmış resmi olmayan bir API'dir. Geliştiricinin şunları yapmasına
#					    olanak tanıyan bir metot ve fonksiyon koleksiyonundan oluşur:
#
#						* Grupları, kanalları ve üyeleri yönetme.
# 						* Mesajlar, belgeler, müzikler, kişiler vb. gönderme.
# 						* Klavyeler yükleme (KeyboardMarkup ve InlineKeyboard).
# 						* Üyeler, dosyalar, gruplar ve kanallar hakkında bilgi alma.
# 						* Daha fazla bilgi için dokümantasyona bakın:
#							  
#						https://github.com/shellscriptx/ShellBot/wiki
#
#					ShellBot, Orijinal API'nin (Telegram) kayıtlı metotlarının varsayılan
# 					adlandırmasını, alanlarını ve değerlerini korur. Metotlar, çağrı ve
# 					yürütme için parametreler ve argümanlar gerektirir. Zorunlu parametreler,
# 					argüman atlanırsa bir hata mesajı döndürür.
#					
#	NOT:				Shell Script dilinde geliştirilmiştir, BASH komut yorumlayıcısını
# 					    kullanır ve yerleşik özelliklerinden en iyi şekilde yararlanarak
# 					    harici paket bağımlılıklarını azaltır.
#-----------------------------------------------------------------------------------------------------------

[[ $_SHELLBOT_SH_ ]] && return 1

if ! awk 'BEGIN { exit ARGV[1] < 4.3 }' ${BASH_VERSINFO[0]}.${BASH_VERSINFO[1]}; then
	echo "${BASH_SOURCE:-${0##*/}}: hata: 'bash 4.3' veya daha yeni bir komut yorumlayıcısı gereklidir." 1>&2
	exit 1
fi
...
[description]='Telegram platformunda bot oluşturmak için resmi olmayan API.'
[version]='6.4.0'
	...
	if command -v $_pkg_ &>/dev/null; then
		if [[ $($_pkg_ --version 2>&1) =~ [0-9]+\.[0-9]+ ]]; then
			if ! awk 'BEGIN { exit ARGV[1] < ARGV[2] }' $BASH_REMATCH $_ver_; then
				printf "%s: hata: '%s %s' veya daha yeni bir sürüm gereklidir.\n" ${_SHELLBOT_[name]} $_pkg_ $_ver_ 1>&2
				exit 1
			fi
		else
			printf "%s: hata: '%s' sürümü alınamadı.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
			exit 1
		fi
	else
		printf "%s: hata: Gerekli olan '%s' paketi eksik.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
		exit 1
	fi
...
# Dosya adı genişletmeyi (globbing) devre dışı bırak.
set -f

readonly _SHELLBOT_SH_=1					# Başlatma
readonly _BOT_SCRIPT_=${0##*/}				# Script
readonly _CURL_OPT_='--silent --request'	# CURL (seçenekler)

# Hatalar
readonly _ERR_TYPE_BOOL_='uyumsuz tür: yalnızca "true" veya "false" desteklenir.'
readonly _ERR_TYPE_INT_='uyumsuz tür: yalnızca tamsayı desteklenir.'
readonly _ERR_TYPE_FLOAT_='uyumsuz tür: yalnızca ondalık sayı (float) desteklenir.'
readonly _ERR_PARAM_REQUIRED_='gerekli seçenek: zorunlu parametrelerin veya argümanların mevcut olup olmadığını kontrol edin.'
readonly _ERR_TOKEN_UNAUTHORIZED_='yetkisiz: tokeni kullanma izniniz olup olmadığını kontrol edin.'
readonly _ERR_TOKEN_INVALID_='geçersiz token: token numarasını kontrol edin ve tekrar deneyin.'
readonly _ERR_BOT_ALREADY_INIT_='işleme izin verilmedi: bot zaten başlatılmış.'
readonly _ERR_FILE_NOT_FOUND_='erişim hatası: dosya okunamadı.'
readonly _ERR_DIR_WRITE_DENIED_='izin reddedildi: dizine yazılamıyor.'
readonly _ERR_DIR_NOT_FOUND_='erişilemedi: dizin bulunamadı.'
readonly _ERR_FILE_INVALID_ID_='geçersiz id: dosya bulunamadı.'
readonly _ERR_UNKNOWN_='bilinmeyen hata: beklenmedik bir hata oluştu. Lütfen sorunu geliştiriciye bildirin.'
readonly _ERR_SERVICE_NOT_ROOT_='erişim reddedildi: root ayrıcalıkları gerektirir.'
readonly _ERR_SERVICE_EXISTS_='servis oluşturma hatası: servis adı zaten mevcut.'
readonly _ERR_SERVICE_SYSTEMD_NOT_FOUND_='etkinleştirme hatası: sistem "systemd" servis yönetimini desteklemiyor.'
readonly _ERR_SERVICE_USER_NOT_FOUND_='kullanıcı bulunamadı: belirtilen kullanıcı hesabı geçersiz.'
readonly _ERR_VAR_NAME_='değişken bulunamadı: tanımlayıcı geçersiz veya mevcut değil.'
readonly _ERR_FUNCTION_NOT_FOUND_='fonksiyon bulunamadı: belirtilen tanımlayıcı geçersiz veya mevcut değil.'
readonly _ERR_ARG_='geçersiz argüman: argüman belirtilen parametre tarafından desteklenmiyor.'
readonly _ERR_RULE_ALREADY_EXISTS_='tanımlama hatası: kural adı zaten mevcut.'
readonly _ERR_HANDLE_EXISTS_='kayıt hatası: geri aramaya (callback) bağlı bir handle zaten mevcut.'
readonly _ERR_CONNECTION_='bağlantı hatası: Telegram ile bağlantı kurulamadı.'
...
MessageError()
{
	...
	# 'BASH_LINENO' değişkeni dinamiktir ve genişletildiği satır numarasını saklar.
	# Bir alt kabuk (subshell) içinde çağrıldığında, bir dizi olarak örneklenir ve her bir
	# dizinin bir kabuk/alt kabuğa karşılık geldiği çeşitli değerleri saklar. Aynı özellikler,
	# çağrıldığı fonksiyonun adının saklandığı 'FUNCNAME' değişkeni için de geçerlidir.
	
	# Çağrı hiyerarşisindeki fonksiyonun indeksini al.
	[[ ${FUNCNAME[1]} == CheckArgType ]] && i=2 || i=1
	
	# Olay türünü oku.
	# TG - Telegram çekirdeği tarafından döndürülen harici hata.
	# API - ShellBot API'si tarafından oluşturulan dahili hata.
	case $1 in
		TG)
	...
	# Dahili hata durumunda script'i/thread'i sonlandır, aksi takdirde 1 döndür
	${assert:-false} && exit 1 || return 1
}

CheckArgType()
{
	# CheckArgType, çağıran fonksiyondan verileri alır ve
	# alınan veriyi parametre tarafından desteklenen türle karşılaştırır.
	# Başarılı olursa '0' döndürülür, aksi takdirde bir hata mesajı
	# döndürülür ve script/thread '1' durumuyla sonlandırılır.
	case $1 in
...
CreateUnitService()
{
	...
	local fail='\033[0;31m[BAŞARISIZ]\033[0;m'
	
	((UID == 0)) || MessageError API "$_ERR_SERVICE_NOT_ROOT_"

	# 'service' modu, Unit hedefinin servise bağlanabilmesi için
	# 'systemd' işlem yönetim sisteminin mevcut olmasını gerektirir.
	if ! which systemctl &>/dev/null; then
		MessageError API "$_ERR_SERVICE_SYSTEMD_NOT_FOUND_"; fi


	# Eğer servis mevcutsa.
	test -e /lib/systemd/system/$service && \
	MessageError API "$_ERR_SERVICE_EXISTS_" "$service"

	# Hedef yapılandırmalarını oluştur.
...
_eof
	...
	[[ $? -eq 0 ]] && {	
		...
		printf '%s başarıyla oluşturuldu!!\n' $service	
		echo -n "Etkinleştiriliyor..."
		...
		systemctl start $service &>/dev/null && {
			...
			echo -e "\nKullanım: sudo systemctl {start|stop|restart|reload|status} $service"
...
ShellBot.init()
{
				...
				-t|--token)
	   			...
	   			-m|--monitor)
					...
					monitor=true
	   				...
	   				;;
				-f|--flush)
					# 'ShellBot.getUpdates' metodu için flush bayrağını ayarlar. Etkinleştirilirse,
					# metodun sadece mevcut güncellemeleri almasını sağlar, JSON nesnelerinin
					# çıkarılmasını ve değişkenlerin başlatılmasını yok sayar.
					flush=true
					shift
					;;
				-s|--service)
    ...
    # Botunuzun kimlik doğrulama token'ını test etmek için basit bir metot. 
    # Parametre gerektirmez. Bot hakkında temel bilgileri bir Kullanıcı nesnesi şeklinde döndürür.
    ShellBot.getMe()
    {
    	# getMe metodunu API adresi ve ardından metot adını geçirerek çağırır.
    	jq_obj=$(curl $_CURL_OPT_ GET $_API_TELEGRAM_/${FUNCNAME#*.})

		# Metodun dönüş durumunu kontrol et
    ...
    # SHELLBOT (FONKSİYONLAR)
	# Telegram API metotlarına çağrılar için fonksiyonları başlatır.
